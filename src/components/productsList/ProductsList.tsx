...
export default function ProductsList() {
    ...
    return (
        <div>
            ...
            <br />
            <div className='flex  font-bold gap-6 justify-center  text-center mx-auto bg-[#fff8eb] p-3'>
                <button onClick={(e) => activeCategory(e)} name='all' className={`p-2 rounded ${active === 'all' && 'bg-[#FB9C16] text-[#fff8eb]'}`}>All</button>
                {categories.map((item, i) => {
                    return (
                        <button onClick={(e) => activeCategory(e)} name={item?.title} key={i} className={`flex gap-1 items-center p-2 rounded ${active === item?.title && 'bg-[#FB9C16] text-[#fff8eb]'}`}>
