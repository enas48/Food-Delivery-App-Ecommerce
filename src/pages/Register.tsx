...
const Register: React.FC<{}> = () => {
  const initialValues: MyFormValues = {username:"", password: '', email: '' };
    ...
    <Helmet title='Register'>
      <>
        ...
        <div className='mx-auto max-w-screen-xl p-6 '>
          ...
          >
            {({ errors, touched }) => (
              <div className="flex w-1/2  flex-col gap-6  p-5 py-9 mx-auto bg-[#fff8eb]  rounded-lg">
                <Form className='flex   flex-col gap-6'>
               
                <label htmlFor="username" className='font-medium text-slate-700'>username</label>
                  <Field id="username" name="username" className={` ${errors.username && touched.username ? 'border-rose-600' : 'border-grey-800'} transition ease-in-out delay-150 bg-transparent py-2 mt-[-1.2rem]  outline-none border-b-2  focus:border-black active:border-black`} placeholder="Enter your username" />
                  {errors.username && touched.username ? <div className='text-rose-600'>{errors.username}</div> : null}
                  ...
                  <Button className='bg-[#FB9C16] mx-auto text-white text-nowrap rounded p-2 mt-2 w-max px-5' type='submit' >Signup</Button>
                </Form>
